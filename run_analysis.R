...
features  <-
  read.table(file.path(path_rf, "features.txt"),header = FALSE)
...
AverageDataSetFinal <-
  aggregate(. ~ MegaDataSet$Subject + MegaDataSet$ActivityName, MegaDataSet, mean)
AverageDataSetFinal <-
  AverageDataSetFinal[order(AverageDataSetFinal$Subject,AverageDataSetFinal$Activity),]
...
names(AverageDataSetFinal) <-
  gsub("^t", "Time", names(AverageDataSetFinal))
names(AverageDataSetFinal) <-
  gsub("^f", "Frequency", names(AverageDataSetFinal))
names(AverageDataSetFinal) <-
...
write.table(AverageDataSetFinal, file = "~/documents/coursera/UCI HAR Dataset/tidydata.txt",row.name =
              FALSE)
